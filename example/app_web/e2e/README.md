...
1. Na raiz de `app_web_playwright`, instale o runner de testes:
...
4. Garanta que o servidor esta no ar antes dos testes (script `webServer` no config do Playwright ou `npm start` manual).

O endpoint `GET /api/health` pode ser usado para readiness antes de abrir o browser.

Fluxo da app: login em `/` com usuario `demo` e senha `demo`, depois `/dashboard.html` (sessao via `sessionStorage`).
